...
## State Summary
- **state_summary:** JT7 is being refactored into one unified platform with four disciplined conversation surfaces, one control plane, and explicit shared-context and handoff behavior. Legacy docs assumed a planning-centered surface model and are being replaced with platform, job ops, life, and TA knowledge as the active structure.

## Top Priorities
- **top_priorities:**
  - lock the unified 4-surface platform model into tenant config and docs
...
## Active Risks
- **active_risks:**
  - some legacy docs and filenames still reflect older three-bot or planning-based assumptions
...
## Open Questions
- **open_questions:**
  - what is the exact live session and bot mapping to use once Telegram bot tokens are available?
...
## Required Next Moves
- **required_next_moves:**
  - finish propagating the 4-surface model into current-state, decision, and roadmap artifacts
  - define the remaining routing, handoff, and shared-context schemas/modules required for runtime implementation
  - map live Telegram/OpenClaw surfaces to `jt7_platform_bot`, `jt7_job_ops_bot`, `jrt7_life_bot`, and `jt7_ta_knowledge_bot`
