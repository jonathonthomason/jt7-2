...
async function newContext(headless) {
...
async function runScan() {
  ...
  for (const query of queries) {
    ...
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60000 });
    const result = await scrapePage(page, query);
    reports.push({ source: 'indeed', query: query.keywords, location: query.location, status: result.blocked ? 'blocked' : 'complete', matches: result.jobs.length, title: result.title });
    if (result.blocked) warnings.push(`indeed:${query.keywords}:${query.location} blocked - run setup`);
    jobs.push(...result.jobs);
    await page.close();
  }
  await context.close();
  const payload = { generated_at: new Date().toISOString(), reports, jobs, warnings };
...
(async () => {
  ...
  console.error('Usage: indeed_browser_scan.cjs [setup|scan]');
  process.exit(1);
})().catch(err => {
  console.error(err);
  process.exit(1);
